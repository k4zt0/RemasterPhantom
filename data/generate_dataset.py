...
랜섬웨어 방어 시나리오·칸네리 운영·Phantom 프로토콜·마스터칸네리/TLS 보호,
...
PLAYBOOKS: list[tuple[str, str]] = [
    ...
    ("칸네리 무결성이 물너졌다는 신호는 무엇이며, 그때 해야 할 일은?",
     ...
     "격리(0600) ② 디렉터리 락다운으로 확산 차단 ③ 마스터칸네리 회전으로 새 키 세대 생성 "
     "④ 클린 스냅샷에서 복원 후 새 시드로 재칸네리 ⑤ 서명된 감사 로그 기록."),
    ("마스터칸네리는 무엇이고 파일의 TLS를 어떻게 보호하나요?",
     "마스터칸네리는 전체 시스템의 루트 오브 트러스트입니다. 파일 칸네리 시드를 파생하고, "
    ...
    ("랜섬웨어 공격이 진행 중이라는 징후를 스캔에서 발견했습니다. 대응 순서는?",
     ...
     "전량 격리 ③ Phantom 에이전트 락다운으로 디렉터리 쓰기 제한 ④ 마스터칸네리 회전 "
    ...
    ("TLS 인증서 핀(pin) 검증이 실패했습니다. 어떤 공격을 의심해야 하나요?",
     ...
     "인증서 도용을 의심합니다. 연결을 즉시 끊고, 마스터칸네리로 래핑된 TLS 자료의 "
    ...
    ("파일 시그니처 기준선(baseline)이 변조됐다는 경고가 떴습니다.",
     ...
     "않으면 공격자가 '변경 없음'으로 속이려 한 것입니다. 즉시 락다운, 마스터칸네리 "
     "회전, 기준선을 신뢰할 수 있는 클린 상태에서 재구성하고, 유출 경로를 조사하세요."),
    ("칸네리와 마스터칸네리의 키 계층 구조를 설명해주세요.",
     "3계층입니다. 최상위 마스터칸네리(루트 시드, Keychain 보관) → 파일 칸네리 시드"
...
SCENARIOS: list[tuple[str, str]] = [
    ("상황: 보호 디렉터리에서 PNG 30개의 시그니처가 동시에 깨졌습니다. 첫 3가지 조치는?",
     ...
     ".quarantine으로 격리해 재암호화(이중 피해) 방지 3) Phantom 락다운 + 마스터칸네리 "
    ...
    ("상황: 외부에서 남아도는 키 파일로 침입을 시도하는 로그가 포착됐습니다.",
     "마스터칸네리를 즉시 회전해 노출 가능성이 있는 키 세대를 폐기하고, 모든 파일을 새 "
