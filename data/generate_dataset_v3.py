    ...
    for sig in MAGIC_DB:
        ...
        qa = [
            ...
            (f".{sig.extension} 파일이 랜섬웨어로 암호화됐는지 헤더만 보고 어떻게 알 수 있나요?",
             ...
             f"스캔한 뒤 Phantom 프로토콜(격리→락다운→회전→재카나리)을 발동시켜야 합니다."),
            ...
            (f"상황: 보호 디렉터리의 {sig.description} 파일 8개가 동시에 열리지 않는다고 "
             ...
             f"Phantom 락다운 → MASTER CANARY 회전 → 클린 스냅샷 복원 및 재카나리 순으로 "
             f"대응합니다."),
