...
- **학습 데이터**: 9,923개 합성 지시-응답 쌍 (v5) — 매직 바이트 식별(MAGIC_DB 59종 전수 드릴), 변종 대비·쌍방향 선택 드릴, 부분 접두 식별, 무결성 검증 절차, 칵네리 운영, Phantom 폴리백 플레이북, TLS 보호, 공격 요청 거부 가드레일, 미등록 시그니처 "모름 인정" 보정 + 실존 매직 20종 + 근접 hex(near-miss) 판별
- **학습 방식**: LoRA (r=16, 전체 선형층), 전체 시퀀스 SFT, bf16, 2 epoch · 완전 중복 없는 구성 (v4의 5,306개 대비 +87%)

## 평가 (scripts/evaluate.py, 105문항)

| 범주 | v3.1 | v4 | **v5** |
|---|---|---|---|
| 시그니처 상식 (59문항) | 86.4% | 93.2% | **100%** |
| 헤더→포맷 식별 (20문항) | 95.0% | 90.0% | **100%** |
| 가드레일 거부 (10문항) | 90.0% | 90.0% | **100%** |
| 미등록 질의 정직 응답 (16문항) | 75.0% | 75.0% | **93.8%** |
| **종합 (미등록 제외)** | **88.8%** | **92.1%** | **100%** |

v1/v2(5,880/23,720개) 대비 개선 폭: 헤더 식별 30%→100%, 미등록 질의 할루시네이션 100%→6.2%.
