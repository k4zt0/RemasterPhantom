...
카나리 기반 파일 시그니처 보호 LLM 에이전트 — 파일의 매직 바이트 무결성을 검증하고, 랜섬웨어로부터 시그니처를 카나리 암호화로 보호하며, 카나리 무결성 붕괴 시 자동 폴리백(Phantom 프로토콜)으로 대응하고, MASTER CANARY로 TLS 자료까지 보호한다. 방어 전용 프로젝트다.
...
```
              ...
              │ 시그니처  │  │ 카나리  │  │ MASTER CANARY│  폴리백
              │ 감시/검증 │  │ 암·복호 │  │ +TLS보호  │  격리→회전→복구
              └────┬─────┘  └───┬────┘  └─────┬─────┘
                   │            │             │
        매직 바이트 DB      카나리 보관소    MASTER CANARY
        +해시 기준선        (HMAC 서명)     (Keychain/epoch)
```

**신뢰 계층 (3계층 키 구조)**
1. **MASTER CANARY** — 루트 시드. macOS Keychain 우선 보관. 파일 카나리 시드와 TLS 래핑 키를 파생한다. 침입 의심 시 회전(rotate)해 키 세대를 갱신한다.
2. **파일 카나리** — 파일별 HMAC 카나리 토큰 → HKDF로 AES-256-GCM 키 유도. 파일 헤더(매직 바이트 포함 첫 4KB)를 암호화하고 난수로 덮어쓴다. 암호문만으로는 파일 형식을 알 수 없다.
3. **감사 로그/기준선** — 모든 보호·복구 작업은 HMAC 서명 로그에 기록되며, 무결성 기준선도 서명돼 공격자가 "변경 없음"을 위조하지 못하게 한다.

**Phantom 프로토콜 (카나리 무결성 붕괴 시 자동 대응)**
1. 격리 — 변조 파일을 `.quarantine/`으로 이동 (0600)
2. 락다운 — 디렉터리 쓰기 권한 제한으로 확산 차단
3. 회전 — MASTER CANARY rotate, 유출 가능 키 세대 폐기
4. 재카나리 — 클린 스냅샷에서 복원 후 새 시드로 재보호
5. 감사 — 서명된 로그 기록
...
- 데이터: `data/generate_dataset_v5.py` → 9,923개 지시-응답 쌍 (`data/remasterphantom_sft_v5.jsonl`, 중복 없음 + 미등록 시그니처·실존 매직 보정)
- 방식: LoRA (r=16, 전체 선형층) · 전체 시퀀스 SFT · bf16 · seq 512 · 2 epoch

**정확도 평가**

```bash
.venv/bin/python scripts/evaluate.py --model outputs/RemasterPhantom-v5-merged
# 시그니처 상식 100% · 헤더 식별 100% · 가드레일 100% · 미등록 정직 응답 93.8% (105문항)
```
...
```bash
pytest tests/ -q    # 20개 — 시그니처/카나리/MASTER CANARY/Phantom 통합
```
...
- 카나리 보관소·기준선은 HMAC 서명으로 보호하지만, MASTER CANARY 루트 시드 자체의 보관(Keychain/HSM)이 최종 안전을 결정한다.
