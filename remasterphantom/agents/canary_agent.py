"""Canary 에이전트 — 파일 헤더 칸네리 암호화/복호화 담당."""
...
class CanaryAgent:
    ...
    def protect(self, path: str | Path) -> dict:
        """파일 헤더를 칸네리로 보호. 이미 보호된 파일은 건드리지 않는다."""
        p = Path(path)
    ...
    def release(self, path: str | Path) -> dict:
        """복호화 수행. 칸네리 붕괴 시 CanaryTamperedError를 위로 전파한다."""
        p = Path(path)
        try:
    ...
    def status(self, path: str | Path) -> dict:
        p = Path(path)
        state = self.cipher.check_integrity(p)
        advice = self.advisor.advise(f"칸네리 상태 {state} — {p}",
